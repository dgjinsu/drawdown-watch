...
class NotificationControllerTest {
    ...
    @MockitoBean
    private NotificationSettingUseCase notificationSettingService;
    ...
    private NotificationSettingResponse buildEmailResponse(Long id) {
        return new NotificationSettingResponse(id, "EMAIL", null, null, null,
                "user@example.com", true, LocalDateTime.now());
    }

    private NotificationSettingResponse buildDiscordResponse(Long id) {
        return new NotificationSettingResponse(id, "DISCORD", null, null,
                "https://discord.com/api/webhooks/test-webhook-url***", null, true, LocalDateTime.now());
    }
    ...
    void create_EMAIL채널_201반환() throws Exception {
        ...
        given(notificationSettingService.create(anyLong(), any())).willReturn(buildEmailResponse(1L));
    ...
    void create_DISCORD채널_201반환() throws Exception {
        ...
        given(notificationSettingService.create(anyLong(), any())).willReturn(buildDiscordResponse(1L));
    ...
    void create_중복EMAIL채널_409반환() throws Exception {
        ...
        given(notificationSettingService.create(anyLong(), any()))
                .willThrow(new BusinessException(ErrorCode.DUPLICATE_NOTIFICATION_SETTING));
    ...
    void toggleEnabled_EMAIL설정_토글성공() throws Exception {
        ...
        given(notificationSettingService.toggleEnabled(anyLong(), anyLong()))
                .willReturn(buildEmailResponse(1L));
    ...
    void toggleEnabled_존재하지않는설정_404반환() throws Exception {
        ...
        stubValidToken(1L);
        given(notificationSettingService.toggleEnabled(anyLong(), anyLong()))
                .willThrow(new BusinessException(ErrorCode.NOTIFICATION_SETTING_NOT_FOUND));
    ...
    void toggleEnabled_타인설정접근_403반환() throws Exception {
        ...
        stubValidToken(1L);
        given(notificationSettingService.toggleEnabled(anyLong(), anyLong()))
                .willThrow(new BusinessException(ErrorCode.WATCHLIST_ACCESS_DENIED));
    ...
    void test_EMAIL테스트발송_성공() throws Exception {
        // Given
        stubValidToken(1L);
        given(notificationSettingService.sendTest(anyLong(), anyLong()))
                .willReturn("테스트 알림 발송 성공");
    ...
    void test_EMAIL발송실패_실패메시지반환() throws Exception {
        ...
        stubValidToken(1L);
        given(notificationSettingService.sendTest(anyLong(), anyLong()))
                .willReturn("테스트 알림 발송 실패: SMTP 연결 실패");
    ...
    void test_DISCORD발송실패_실패메시지반환() throws Exception {
        ...
        stubValidToken(1L);
        given(notificationSettingService.sendTest(anyLong(), anyLong()))
                .willReturn("테스트 알림 발송 실패: Discord webhook 오류");
